-- Include the owning extension for extension-defined column types in every
-- transactional schema snapshot. This remains a separate forward migration so
-- databases that already applied the original schema-message migrations keep
-- their recorded checksums valid.
